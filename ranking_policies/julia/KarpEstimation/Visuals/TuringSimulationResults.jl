...
################################### Plot 1 - basic ###################################
# Group by nparam, nsim and keep the row with max LL
df1 = @chain df begin
    groupby([:nparam, :nsim])
    combine(sdf -> last(groupby(sdf, :LL, sort=true)), _)
end
...
df1 = @chain df begin
    ...
    @transform(
        :log_σμ²_biasperc = log10.(:σα²_biasperc_avg .- minimum(:σα²_biasperc_avg) .+ 1e-10),
        :log_σμ²_abiasperc = log10.(:σα²_abiasperc_avg)
    )
end
minimum(df1.σα²_biasperc_avg)
minimum(df1.σα²_biasperc_avg .- minimum(df1.σα²_biasperc_avg) .+ 1e-10)
# Save df1 to csv
...
PlotlyJS.plot(
    PlotlyJS.heatmap(
        filter(row -> row.log_σμ²<5 && row.log_σα²<5, df1),
        x=:log_σμ², y=:log_σα², z=:σα²_biasperc_avg), 
        Layout(title="Avg Bias Percentage of σα²",
            xaxis_title="log_σμ²", yaxis_title="log_σα²")
)
minimum(df1.σα²_biasperc_avg)
StatsPlots.histogram(df1.σα²_biasperc_avg)
...
PlotlyJS.plot(
    PlotlyJS.heatmap(df1,
        x=:log_σμ², y=:log_σα², z=:log_σμ²_biasperc, title="Avg Absolute Bias Percentage of σα²")
)
...
sim_results_file = "C:\\Users\\A\\github\\karp_ranking_policies\\data\\temp\\turing_simulation_output\\Nsigma-param-sim-seed_10-10-10-10\\simulation_df.csv"
df = DataFrame(CSV.File(sim_results_file))
...
params = names(df)[8:9]
for param in params
    df[!, param * " bias"] = df[!, param * "_est"] - df[!, param]
    df[!, param * " % bias"] = df[!, param * " bias"] ./ df[!, param]
    df[!, param * " abs bias"] = abs.(df[!, param * " bias"])
    df[!, param * " % abs bias"] = abs.(df[!, param * " % bias"])
end
